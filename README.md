...
I wrote 13 unit tests to cover the features in forms.py and views.py. The tests can be located in [todo/test_forms.py](todo/test_forms.py) and [todo/test_views.py](todo/test_views.py). The tests are started with `python3 manage.py test` on the terminal. The screenshot below shows that all tests passed successfully in 2.081s.

![Django unit tests results displayed on terminal, 0 fails](documentation/validator/django-unit-tests.png)
